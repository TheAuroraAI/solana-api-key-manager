...
export function hashApiKey(rawKey: string): Buffer {
  if (!rawKey) throw new Error("rawKey must not be empty");
...
export function isValidPermissions(mask: number): boolean {
  return Number.isInteger(mask) && (mask & ~Permission.ALL) === 0 && mask >= 0;
}
 ...
 *
 * Wraps all 10 program instructions with typed methods, handles PDA derivation,
...
export class ApiKeyManagerSDK {
  ...
  constructor(
    ...
    this.provider = new anchor.AnchorProvider(connection, anchorWallet as anchor.Wallet, {
  ...
  public async fetchServiceConfigByAddress(
    ...
    try {
      const account = await (this.program.account as Record<string, { fetch: (addr: PublicKey) => Promise<ServiceConfigAccount> }>).serviceConfig.fetch(
        address
      );
      return account as ServiceConfigAccount;
    } catch (e: unknown) {
      // Return null only for "account does not exist" errors
      const msg = e instanceof Error ? e.message : String(e);
      if (msg.includes("Account does not exist") || msg.includes("Could not find")) {
        return null;
      }
      throw e; // Re-throw network/deserialization errors
    }
  }
  ...
  public async fetchApiKeyByAddress(
    ...
    try {
      const account = await (this.program.account as Record<string, { fetch: (addr: PublicKey) => Promise<ApiKeyAccount> }>).apiKey.fetch(address);
  ...
  public async fetchAllApiKeysForService(
    ...
    const accounts = await (this.program.account as Record<string, { all: (filters: unknown[]) => Promise<{ publicKey: PublicKey; account: ApiKeyAccount }[]> }>).apiKey.all([
      {
        memcmp: {
    ...
    return accounts.map((a: { publicKey: PublicKey; account: ApiKeyAccount }) => ({
  ...
  /** Validate a service or key name/label (1-32 bytes). */
  private validateName(name: string, fieldName: string): void {
    const len = Buffer.byteLength(name, "utf-8");
    if (len === 0 || len > 32) {
      throw new Error(`${fieldName} must be 1-32 bytes (got ${len} bytes)`);
    }
  }

  /** Validate a permission bitmask (0-15 for u16 range, valid bits only). */
  private validatePermissions(mask: number): void {
    if (!Number.isInteger(mask) || mask < 0 || mask > 15) {
      throw new Error(`Permission mask must be an integer 0-15 (got ${mask})`);
    }
  }

  /** Validate a u32 value is within range and positive. */
  private validateU32(value: number, fieldName: string, minValue: number = 1): void {
    if (!Number.isInteger(value) || value < minValue || value > 4294967295) {
      throw new Error(`${fieldName} must be an integer ${minValue}-4294967295 (got ${value})`);
    }
  }
   ...
   * @param params.name - Human-readable service name (1-32 bytes).
   ...
   * - Name is empty or exceeds 32 bytes (`InvalidName`)
  ...
  public async initializeService(params: {
    ...
    this.validateName(params.name, "Service name");
    this.validateU32(params.maxKeys, "maxKeys");
    if (params.maxKeys > 10000) {
      throw new Error("maxKeys must be <= 10,000");
    }
    this.validateU32(params.defaultRateLimit, "defaultRateLimit");
    const validWindows = [60, 3600, 86400];
    if (!validWindows.includes(params.rateLimitWindow)) {
      throw new Error(`rateLimitWindow must be one of ${validWindows.join(", ")}`);
    }
   ...
   * @param params.name - New service name (1-32 bytes).
  ...
  public async updateService(params: {
    ...
    // Client-side validation to save transaction fees on invalid inputs
    if (params.name != null) {
      this.validateName(params.name, "Service name");
    }
    if (params.maxKeys != null) {
      this.validateU32(params.maxKeys, "maxKeys");
      if (params.maxKeys > 10000) {
        throw new Error("maxKeys must be <= 10,000");
      }
    }
    if (params.defaultRateLimit != null) {
      this.validateU32(params.defaultRateLimit, "defaultRateLimit");
    }
    if (params.rateLimitWindow != null) {
      const validWindows = [60, 3600, 86400];
      if (!validWindows.includes(params.rateLimitWindow)) {
        throw new Error(
          `rateLimitWindow must be one of ${validWindows.join(", ")} (got ${params.rateLimitWindow})`
        );
      }
    }
   ...
   * @param params.label - Human-readable label (1-32 bytes). Defaults to `"default"`.
   ...
   *
   * @throws Will throw an `AnchorError` if:
   * - Label is empty or exceeds 32 bytes (`InvalidName`)
  ...
  public async createKey(params?: {
    ...
    this.validateName(label, "Key label");
    this.validatePermissions(permissionsMask);
    if (rateLimit !== null) {
      this.validateU32(rateLimit, "rateLimit");
    }
    if (params?.expiresAt != null && params.expiresAt <= Math.floor(Date.now() / 1000)) {
  ...
  public async createKeyFromRaw(
    ...
    if (!rawKey || rawKey.length < 32) {
      throw new Error("Raw key must be at least 32 characters for security (short keys are trivially brute-forceable since the hash is public on-chain)");
    }
  ...
  public async checkPermission(
    ...
    if (!Number.isInteger(requiredPermission) || requiredPermission <= 0 || requiredPermission > 15) {
      throw new Error(
        `requiredPermission must be an integer 1-15 (got ${requiredPermission})`
      );
    }
  ...
  public async updateKey(
    ...
    // Client-side validation to save transaction fees on invalid inputs
    if (params.permissionsMask != null) {
      this.validatePermissions(params.permissionsMask);
    }
    if (params.rateLimit != null) {
      this.validateU32(params.rateLimit, "rateLimit");
    }
    if (
      ...
      throw new Error("expiresAt must be in the future (or 0 to clear)");
    }
  ...
  public async simulateValidateKey(
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      // Propagate network errors instead of treating them as "invalid key"
      if (
        ...
        throw new Error(`Network error during key validation: ${msg}`);
      }
      return { valid: false, error: msg || "Simulation failed" };
    }
  }
  ...
  public async simulateCheckPermission(
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      // Propagate network errors instead of treating them as "no permission"
      if (
        msg.includes("ECONNREFUSED") ||
        msg.includes("timeout") ||
        msg.includes("fetch failed") ||
        ...
        throw new Error(`Network error during permission check: ${msg}`);
      }
      return { hasPermission: false, error: msg || "Simulation failed" };
    }
  }
  ...
  public async rotateKey(
    ...
    // Client-side validation
    if (params?.newLabel != null) {
      this.validateName(params.newLabel, "New key label");
    }
