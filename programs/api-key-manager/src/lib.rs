...
pub mod api_key_manager {
    ...
    pub fn check_permission(ctx: Context<ValidateKey>, required_permission: u16) -> Result<()> {
        require!(required_permission > 0, ApiKeyError::InvalidPermissions);
        require!(permissions::is_valid(required_permission), ApiKeyError::InvalidPermissions);
    ...
    pub fn rotate_key(
        ...
        // Check expiry — don't allow rotating an expired key
        let clock = Clock::get()?;
        if old_key.expires_at > 0 {
            require!(
                clock.unix_timestamp < old_key.expires_at,
                ApiKeyError::KeyExpired
            );
        }
...
pub struct RecordUsage<'info> {
    ...
    #[account(
        ...
        seeds = [b"apikey", service_config.key().as_ref(), &key_hash],
        bump = api_key.bump,
        constraint = api_key.service == service_config.key() @ ApiKeyError::InvalidService,
    )]
    pub api_key: Account<'info, ApiKey>,
...
pub struct RotateKey<'info> {
    ...
    #[account(
        ...
        constraint = old_api_key.service == service_config.key() @ ApiKeyError::InvalidService,
    )]
